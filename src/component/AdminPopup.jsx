...
function AdminPopup({ imgSrc, id, name, onClick }) {
  return (
    <section
      className="bg-green-800 text-white rounded-lg overflow-hidden p-2 flex flex-col gap-2 cursor-pointer"
      onClick={() => onClick(id)}
    >
      ...
      <h2 className="font-semibold tracking-wide text-xl">
        Help us to Care{" "}
        {name.replace(/\b[A-Z]{1,2}\d{1,2}\b/, "").trim() + " Gaumata"}
      </h2>
